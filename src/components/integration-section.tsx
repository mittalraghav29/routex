...
import { Route, Sliders, ShieldCheck, Zap, Target, Code } from "lucide-react";
          ...
          <div className="bg-muted/25 group relative mx-auto max-w-[22rem] items-center justify-between space-y-6 [mask-image:radial-gradient(ellipse_50%_50%_at_50%_50%,#000_70%,transparent_100%)] sm:max-w-md">
            ...
            <div className="absolute inset-0 m-auto flex size-fit justify-center gap-2">
              <IntegrationCard
                ...
                isCenter={true}
              >
                <Route className="size-8 text-indigo-600 dark:text-indigo-400" />
              </IntegrationCard>
            </div>
          </div>
          <div className="mx-auto mt-12 max-w-lg space-y-6 text-center">
            <h2 className="text-balance text-3xl font-semibold md:text-4xl">
              Integrate with any model: the smart routing platform
            </h2>
            <p className="text-muted-foreground">
              RouteX automatically routes your tasks to the best AI model from
              leading providers for optimal cost and performance.
            </p>

            <Button variant="outline" size="sm" asChild>
              <Link href="/app">Start Building with RouteX</Link>
            </Button>
          </div>
        </div>
      </div>
